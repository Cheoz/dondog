...
class Core(commands.Cog, CoreLogic):
    ...
    async def info(self, ctx: commands.Context):
        ...
        about = _(
            "This bot is an instance of [DonDog Discord bot]({}) "
            "created by [Puh]({}) - Embrace the dog({}).\n\n"
            "DonDog has been developed by highly skilled dogs "
            "and they teamed up with the humans. [DonDog]({}) "
            "We out!\n\n"
            "(c) Cog Creators"
        ...
        embed.add_field(name=_("Dog version"), value=red_version)
        if outdated:
            embed.add_field(
        ...
        embed.add_field(name=_("About DonDog"), value=about, inline=False)

        embed.set_footer(
            text=_("Bringing dogs since 02 Jan 2016 (over {} days ago!)").format(days_since)
        )
        try:
    ...
    async def locale(self, ctx: commands.Context, locale_name: str):
        ...
        """
        red_dist = pkg_resources.get_distribution("dondog-discordbot")
        red_path = Path(red_dist.location) / "dondogbot"
    ...
    async def debuginfo(self, ctx: commands.Context):
        ...
        if await ctx.embed_requested():
            ...
            e.title = "Debug Info for DonDog"
            e.add_field(name="DonDog version", value=redver, inline=True)
...
async def license_info_command(ctx):
    ...
    message = (
        "This bot is an instance of Don Dog The Discord Bot \n"
        "DonDog is a bot developed by highly skilled dogs and humans "
